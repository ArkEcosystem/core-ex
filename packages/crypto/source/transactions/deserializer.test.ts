...
describe("Transaction serializer / deserializer", ({ it, assert }) => {
	...
	it("[version 1] - ser/deserialize - multi signature - should ser/deserialize a multisig registration", () => {
		...
		assert.equal(transaction.serialized.toString("hex"), 
	...
	it("[version 1] - ser/deserialize - multi signature - should not deserialize a malformed signature", () => {
		...
		assert.throws(() => TransactionFactory.fromBytes(transaction.serialized), err => err instanceof InvalidTransactionBytesError);
	});
	...
	it("[version 2] ser/deserialize - multi payment - should fail if more than hardcoded maximum of payments", () => {
		...
		assert.throws(() => multiPayment.addPayment(Address.fromPassphrase("recipientBad"), "1"), err => err instanceof Errors.MaximumPaymentCountExceededError);
	...
	it("deserialize - others - should throw if type is not supported", () => {
		...
		assert.throws(() => Deserializer.deserialize(serialized), err => err instanceof UnkownTransactionError);
	});
	...
	it("serialize - others - should throw if type is not supported", () => {
		...
		assert.throws(() => TransactionFactory.fromData(transactionWrongType), err => err instanceof UnkownTransactionError);
	});
	...
	it("getBytesV1 - should throw for unsupported version", () => {
		...
		assert.throws(() => Serializer.getBytes(transaction), err => err instanceof TransactionVersionError);
