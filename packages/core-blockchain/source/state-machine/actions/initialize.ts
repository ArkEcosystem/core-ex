		...
		try {
			...
			if (block.data.height === 1) {
				if (block.data.payloadHash !== this.configuration.get("network.nethash")) {
					this.logger.error(`FATAL: The genesis block payload hash (${block.data.payloadHash}) is different from configured the nethash (${this.configuration.get("network.nethash")})`);
