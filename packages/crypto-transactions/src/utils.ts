 ...
 */
export const formatSatoshi = (amount: BigNumber, configManager: ConfigManager): string => {
 ...
 */
export const isIdException = (id: number | string | undefined, configManager: ConfigManager): boolean => {
    if (!id) {
        return false;
    }
...
export const isException = (
    blockOrTransaction: { id?: string; transactions?: ITransactionData[] },
    ...
    return isIdException(blockOrTransaction.id, configManager);
};

export const isGenesisTransaction = (id: string, configManager: ConfigManager): boolean => {
    const network: number = configManager.get("network.pubKeyHash");
...
export const maxVendorFieldLength = (configManager: ConfigManager, height?: number): number =>
    configManager.getMilestone(height).vendorFieldLength;

export const isSupportedTransactionVersion = (version: number, configManager: ConfigManager): boolean => {
