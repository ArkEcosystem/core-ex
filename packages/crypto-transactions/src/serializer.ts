...
export class TransactionSerializer {
    ...
    public getBytes(transaction: ITransactionData, options: ISerializeOptions = {}): Buffer {
        ...
        if (
            ...
            isSupportedTransactionVersion(version, this.#config)
        ) {
            if (version === 1) {
        ...
        if (transaction.senderPublicKey) {
            ...
            if (isBrokenTransaction || (transaction.recipientId && transaction.type !== 1 && transaction.type !== 4)) {
                const recipientId =
                    ...
                    Address.fromPublicKey(transaction.senderPublicKey, {
                        pubKeyHash: transaction.network!,
                    });

                const recipient = Address.toBuffer(recipientId, {
                    pubKeyHash: this.#config.get("network.pubKeyHash"),
                }).addressBuffer;
    ...
    private serializeCommon(transaction: ITransactionData, buffer: ByteBuffer): void {
        ...
        buffer.writeByte(transaction.network || this.#config.get("network.pubKeyHash"));
    ...
    private serializeVendorField(transaction: ITransaction, buffer: ByteBuffer): void {
    ...
    private serializeSignatures(
        ...
        if (transaction.signatures) {
            if (transaction.version === 1 && isException(transaction, this.#config)) {
