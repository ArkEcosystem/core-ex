...
import { devnet, mainnet, testnet } from "../../test/networks";
import { ConfigManager } from "../config";
import { TransactionType } from "../enums";
import { TransactionValidator } from "./";

//@ts-ignore
let configManager = new ConfigManager(testnet);
let transactionValidator = new TransactionValidator(configManager);
let ajv = (transactionValidator as any).ajv;

const setupWithNetwork = (network) => {
    configManager = new ConfigManager(network);
    transactionValidator = new TransactionValidator(configManager);
    ajv = (transactionValidator as any).ajv;
};

beforeEach(() => setupWithNetwork(testnet));
