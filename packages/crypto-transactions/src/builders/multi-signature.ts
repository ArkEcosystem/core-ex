...
import { IMultiSignatureAsset, ITransactionData } from "../contracts";
    ...
    public participant(publicKey: string): MultiSignatureBuilder {
        if (this.data.asset && this.data.asset.multiSignature) {
            ...
            if (publicKeys.length <= 16) {
                ...
                this.data.fee = new Two.MultiSignatureRegistrationTransaction(this.config, this.verifier).getStaticFee({
                    data: this.data,
                });
            }
        }
