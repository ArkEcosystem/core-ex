import { Address, Contracts, Keys } from "@arkecosystem/crypto-identities";
...
export abstract class TransactionBuilder<TBuilder extends TransactionBuilder<TBuilder>> {
	...
	public sign(passphrase: string): TBuilder {
		const keys: Contracts.KeyPair = Keys.fromPassphrase(passphrase);
		return this.signWithKeyPair(keys);
	}

	public signWithWif(wif: string, networkWif?: number): TBuilder {
		const keys: Contracts.KeyPair = Keys.fromWIF(wif, {
	...
	private signWithKeyPair(keys: Contracts.KeyPair): TBuilder {
	...
	private secondSignWithKeyPair(keys: Contracts.KeyPair): TBuilder {
		this.data.secondSignature = this.signer.secondSign(this.getSigningObject(), keys);
		return this.instance();
	}

	private multiSignWithKeyPair(index: number, keys: Contracts.KeyPair): TBuilder {
