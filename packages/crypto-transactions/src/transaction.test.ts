...
import { devnet } from "../test/networks";
import { Two } from "./types";

let verifier, builderFactory, factory, config;
...
describe("Transaction", () => {
    describe("should deserialize correctly some tests transactions", () => {
        describe("mainnet", () => {
            ...
            for (const tx of txs) {
                it(`txid: ${tx.id}`, () => {
                    const newtx = factory.fromData(tx);
    ...
    describe("static fees", () => {
        it("should update fees on milestone change", () => {
            ...
            config.set("milestones", devnet.milestones);
            config.buildConstants();
            config.setHeight(height);

            let { staticFees } = config.getMilestone().fees;
            expect(new Two.TransferTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(1234),
            );
            expect(new Two.SecondSignatureRegistrationTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.secondSignature),
            );
            expect(new Two.DelegateRegistrationTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.delegateRegistration),
            );
            expect(new Two.VoteTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.vote),
            );
            expect(new Two.MultiSignatureRegistrationTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.multiSignature),
            );
            expect(new Two.IpfsTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.ipfs),
            );
            expect(new Two.MultiPaymentTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.multiPayment),
            );
            expect(new Two.DelegateResignationTransaction(config, verifier).getStaticFee({ height })).toEqual(
                BigNumber.make(staticFees.delegateResignation),
            );

            config.setHeight(1);
            staticFees = config.getMilestone().fees.staticFees;

            expect(new Two.TransferTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.transfer),
            );
            expect(
                new Two.SecondSignatureRegistrationTransaction(config, verifier).getStaticFee({ height: 1 }),
            ).toEqual(BigNumber.make(staticFees.secondSignature));
            expect(new Two.DelegateRegistrationTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.delegateRegistration),
            );
            expect(new Two.VoteTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.vote),
            );
            expect(new Two.MultiSignatureRegistrationTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.multiSignature),
            );
            expect(new Two.IpfsTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.ipfs),
            );
            expect(new Two.MultiPaymentTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.multiPayment),
            );
            expect(new Two.DelegateResignationTransaction(config, verifier).getStaticFee({ height: 1 })).toEqual(
                BigNumber.make(staticFees.delegateResignation),
            );
    ...
    describe("toString", () => {
        it("should describe v1 transaction", () => {
            ...
            const senderAddress = Address.fromPassphrase("sender's secret", { pubKeyHash: 30 });
            const recipientAddress = Address.fromPassphrase("recipient's secret", { pubKeyHash: 30 });
